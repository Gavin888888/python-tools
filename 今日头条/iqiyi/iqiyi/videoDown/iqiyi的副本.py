...
def main_other(file_name,file_url):
   print(file_url)
   print(file_name)
   dir_path = mkdir("download_videos")  # 创建媒体名命名的文件夹

   '''获取m3u8文件的内容并放入内存'''
   m3u8_target = file_url
   target = '/'.join(m3u8_target.split('/')[:-1])
   print(target)
   m3u8_res = requests.get(m3u8_target, headers)
   if m3u8_res.status_code != 200:
   ...
   media_url_list = read_m3u8(m3u8_content)
   flag = download_media(media_url_list, target, file_name)  # 用于校验是否成功下载媒体文件
   if flag:
      print('媒体下载完成')
   else:
      print('媒体下载失败')
